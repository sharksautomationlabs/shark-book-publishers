...
export default function AuthorsSection(): React.ReactElement {
  return (
    <section
        className={`relative w-full flex flex-col items-center justify-center bg-white py-20 px-4 md:px-10 lg:px-16 text-center ${barlowCondensed.className}`}
    >
      {/* ===== Title ===== */}
      <h2 className="text-[#111] font-semibold mb-4 capitalize leading-[clamp(100px,5.6vw,85px)] text-[clamp(36px,5vw,75px)]">
        Meet Our Authors
      </h2>

      {/* ===== Description ===== */}
      <p className="text-black leading-[1.5] mx-auto w-[90%] sm:w-[80%] md:w-[70%] lg:w-[55%] max-w-[750px] mt-4 mb-[5%] text-[clamp(18px,1.5vw,24px)] font-light">
      ...
      {/* ===== Authors Grid ===== */}
      <div className="flex flex-wrap justify-center items-start gap-10 md:gap-8">
        {authors.map((author) => (
          <div
            key={author.id}
            className="w-[clamp(350px,31vw,390px)] text-left relative transition-all duration-300 ease-in-out"
          >
            {/* ===== Image Background Wrapper ===== */}
            <div className="relative w-full h-[clamp(400px,50vw,520px)] overflow-hidden flex justify-center items-end pt-[30px] rounded-[20px] bg-transparent">
            ...
            {/* ===== Author Info ===== */}
            <div className="text-left mt-5 px-[10px] md:px-5">
              <h3 className="text-[#063f4a] font-semibold capitalize leading-[1.1] text-[clamp(22px,3vw,40px)]">
                {author.name}
              </h3>
              <p
                className={`whitespace-pre-line mt-[10px] font-normal leading-[1.6] text-[clamp(15px,1.2vw,18px)] text-black ${barlow.className}`}
              >
                {author.description}
              </p>
            </div>
